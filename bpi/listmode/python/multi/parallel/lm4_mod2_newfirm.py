...
filelength=100000
max_buffer_size = 5461
#restartpoint = 25
#maxfiletime = 50

def lm4_mod2_newfirm(all_morpho, sn, seconds):
  ...
  #if sn == 'eRC1488':
  #  max_buffer_frac = 50/5461.0
  #  maxfiletime = 50 
  if sn == 'eRC1489':
    max_buffer_frac = 0.50
    maxfiletime = 900
  if sn == 'eRC1490':
    max_buffer_frac = 0.60
    maxfiletime = 900
  if sn == 'eRC1491':
    max_buffer_frac = 0.70
    maxfiletime = 900

  #set event buffer restart point
  restartpoint = max_buffer_size*max_buffer_frac
  ...
  t0file = "/home/vladimir/GODOT/t0/%s%s"%(sn,time.strftime("_lm4_newfirm_t0_%y%m%d_%H%M%S.csv"))
  f=open(t0file,'w')
  ...
  while finishing < 1:
      ...
      lm_data_file = "/home/vladimir/GODOT/data/%s%s.csv"%(sn,time.strftime("_lm4_mod2_newfirm_%y%m%d_%H%M%S"))
