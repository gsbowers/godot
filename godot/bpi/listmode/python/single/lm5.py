...
while finishing < 1:
    ...
    lm_data_file = (time.strftime("/home/vladimir/github/godot/bpi/listmode/python/single/data/lm5_%y%m%d_%H%M%S.csv"))
...
#From lm3 we replaced "clear_statistics" with 0 instead of 1
    ...
    #lm_data = emorpho_io.read(all_morpho, sn, emorpho_io.MA_LISTMODE,2*buffersize,2)
    #countslasttime = lm_data[buffersize-1] & 0x1FF # number of valid events
    ...
    while (length < filelength and finishing < 1 and filefinishing < 1) :
        ...
        lm_data = emorpho_io.read(all_morpho, sn, emorpho_io.MA_LISTMODE, 2*buffersize,2)
        lm_counts = lm_data[buffersize-1] & 0x1FF # number of valid events
        ...
        #exit loop if ready to write to file
